...
{
	public class Genres_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_Genres> Get_All_KinguinProduct_To_Genres() { 
		...
		var SQL = "Select KinguinProduct_To_Genres.ID From KinguinProduct_To_Genres Where Genres_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public Genres_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Genres_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Genres Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Genres> GetGenres(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Genres> _TmpReturn = new List<Genres>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Genres Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Genres((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Genres> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Genres> _TmpReturn = new List<Genres>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Genres.ID From Genres Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Genres.ID From Genres Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Genres((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Genres Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Genres Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Genres> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Genres> _TmpReturn = new List<Genres>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Genres.ID From Genres where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Genres Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Genres Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Genres] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Genres] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Genres] Where " + SQLValues;
