...
{
	public class Publishers_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_Publishers> Get_All_KinguinProduct_To_Publishers() { 
		...
		var SQL = "Select KinguinProduct_To_Publishers.ID From KinguinProduct_To_Publishers Where Publishers_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public Publishers_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Publishers_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Publishers Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Publishers> GetPublishers(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Publishers> _TmpReturn = new List<Publishers>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Publishers Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Publishers((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Publishers> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Publishers> _TmpReturn = new List<Publishers>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Publishers.ID From Publishers Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Publishers.ID From Publishers Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Publishers((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Publishers Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Publishers Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Publishers> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Publishers> _TmpReturn = new List<Publishers>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Publishers.ID From Publishers where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Publishers Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Publishers Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Publishers] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Publishers] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Publishers] Where " + SQLValues;
