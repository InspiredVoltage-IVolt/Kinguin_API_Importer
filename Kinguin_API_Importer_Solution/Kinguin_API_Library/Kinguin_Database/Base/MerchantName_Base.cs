...
{
	public class MerchantName_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_MerchantName> Get_All_KinguinProduct_To_MerchantName() { 
		...
		var SQL = "Select KinguinProduct_To_MerchantName.ID From KinguinProduct_To_MerchantName Where MerchantName_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public MerchantName_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public MerchantName_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From MerchantName Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<MerchantName> GetMerchantName(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<MerchantName> _TmpReturn = new List<MerchantName>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From MerchantName Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new MerchantName((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<MerchantName> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<MerchantName> _TmpReturn = new List<MerchantName>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select MerchantName.ID From MerchantName Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select MerchantName.ID From MerchantName Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new MerchantName((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From MerchantName Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From MerchantName Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<MerchantName> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<MerchantName> _TmpReturn = new List<MerchantName>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select MerchantName.ID From MerchantName where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From MerchantName Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From MerchantName Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [MerchantName] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [MerchantName] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [MerchantName] Where " + SQLValues;
