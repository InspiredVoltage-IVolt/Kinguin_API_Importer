...
{
	public class CheapestOfferId_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_CheapestOfferId> Get_All_KinguinProduct_To_CheapestOfferId() { 
		...
		var SQL = "Select KinguinProduct_To_CheapestOfferId.ID From KinguinProduct_To_CheapestOfferId Where CheapestOfferId_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public CheapestOfferId_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public CheapestOfferId_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From CheapestOfferId Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<CheapestOfferId> GetCheapestOfferId(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<CheapestOfferId> _TmpReturn = new List<CheapestOfferId>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From CheapestOfferId Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new CheapestOfferId((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<CheapestOfferId> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<CheapestOfferId> _TmpReturn = new List<CheapestOfferId>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select CheapestOfferId.ID From CheapestOfferId Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select CheapestOfferId.ID From CheapestOfferId Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new CheapestOfferId((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From CheapestOfferId Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From CheapestOfferId Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<CheapestOfferId> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<CheapestOfferId> _TmpReturn = new List<CheapestOfferId>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select CheapestOfferId.ID From CheapestOfferId where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From CheapestOfferId Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From CheapestOfferId Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [CheapestOfferId] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [CheapestOfferId] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [CheapestOfferId] Where " + SQLValues;
