...
{
	public class Systems_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
	...
	private string _LocalConnectionString = "";
		...
		private string __SystemName_Description = "";
	...
	public virtual string PrimaryKey { get { return "ID"; } }		public virtual int? ID
		...
		{
			get { return __SystemName; }
			set { __SystemName = value; }
		}

		public virtual string SystemName_Description
		{
			get { return __SystemName_Description; }
			set { __SystemName_Description = value; }
		}
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	#region Public Parent Tables
		...
		public Systems_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Systems_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Systems Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Systems> GetSystems(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Systems> _TmpReturn = new List<Systems>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Systems Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Systems((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Systems> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Systems> _TmpReturn = new List<Systems>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Systems.ID From Systems Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Systems.ID From Systems Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Systems((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Systems Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Systems Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Systems> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Systems> _TmpReturn = new List<Systems>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Systems.ID From Systems where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Systems Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Systems Where [" + FieldName + "] = @SearchParam";}
...
_Params.Add(new System.Data.SqlClient.SqlParameter("SystemName_Param", this.SystemName));
SQLInsertFields += "[SystemName], ";
SQLValues += "@SystemName_Param, ";
}
...
string _SQL = "Insert Into [Systems] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
if (this.SystemName != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("SystemName_Param", this.SystemName));
SQLInsertFields += "[SystemName] = @SystemName_Param, ";
}
if (this.DateAdded != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("DateAdded_Param", this.DateAdded));
...
string _SQL = "Update [Systems] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Systems] Where " + SQLValues;
