...
{
	public class Languages_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_Languages> Get_All_KinguinProduct_To_Languages() { 
		...
		var SQL = "Select KinguinProduct_To_Languages.ID From KinguinProduct_To_Languages Where Languages_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public Languages_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Languages_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Languages Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Languages> GetLanguages(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Languages> _TmpReturn = new List<Languages>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Languages Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Languages((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Languages> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Languages> _TmpReturn = new List<Languages>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Languages.ID From Languages Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Languages.ID From Languages Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Languages((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Languages Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Languages Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Languages> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Languages> _TmpReturn = new List<Languages>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Languages.ID From Languages where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Languages Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Languages Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Languages] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Languages] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Languages] Where " + SQLValues;
