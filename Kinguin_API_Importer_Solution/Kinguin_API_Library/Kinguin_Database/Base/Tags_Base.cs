...
{
	public class Tags_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_Tags> Get_All_KinguinProduct_To_Tags() { 
		...
		var SQL = "Select KinguinProduct_To_Tags.ID From KinguinProduct_To_Tags Where Tags_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public Tags_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Tags_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Tags Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Tags> GetTags(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Tags> _TmpReturn = new List<Tags>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Tags Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Tags((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Tags> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Tags> _TmpReturn = new List<Tags>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Tags.ID From Tags Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Tags.ID From Tags Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Tags((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Tags Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Tags Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Tags> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Tags> _TmpReturn = new List<Tags>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Tags.ID From Tags where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Tags Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Tags Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Tags] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Tags] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Tags] Where " + SQLValues;
