...
{
	public class Developers_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public List<KinguinProduct_To_Developers> Get_All_KinguinProduct_To_Developers() { 
		...
		var SQL = "Select KinguinProduct_To_Developers.ID From KinguinProduct_To_Developers Where Developers_ID = @IDParam";
	...
	#region Public Parent Tables
		...
		public Developers_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Developers_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Developers Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Developers> GetDevelopers(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Developers> _TmpReturn = new List<Developers>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Developers Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Developers((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Developers> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Developers> _TmpReturn = new List<Developers>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Developers.ID From Developers Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Developers.ID From Developers Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Developers((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Developers Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Developers Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Developers> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Developers> _TmpReturn = new List<Developers>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Developers.ID From Developers where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Developers Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Developers Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Developers] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Developers] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Developers] Where " + SQLValues;
