...
{
	public class Regions_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	#region Public Parent Tables
		...
		public Regions_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Regions_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Regions Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Regions> GetRegions(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Regions> _TmpReturn = new List<Regions>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Regions Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Regions((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Regions> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Regions> _TmpReturn = new List<Regions>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Regions.ID From Regions Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Regions.ID From Regions Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Regions((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Regions Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Regions Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Regions> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Regions> _TmpReturn = new List<Regions>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Regions.ID From Regions where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Regions Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Regions Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Regions] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Regions] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Regions] Where " + SQLValues;
