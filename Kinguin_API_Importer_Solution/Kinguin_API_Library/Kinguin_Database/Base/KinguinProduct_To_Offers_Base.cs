...
{
	public class KinguinProduct_To_Offers_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
	...
	private string _LocalConnectionString = "";
		...
		private int? __Offer_ID;
		private string __Offer_ID_Description = "";
	...
	public virtual string PrimaryKey { get { return "ID"; } }		public virtual int? ID
		...
		public virtual int? Offer_ID
		{
			get { return __Offer_ID; }
			set { __Offer_ID = value; }
		}

		public virtual string Offer_ID_Description
		{
			get { return __Offer_ID_Description; }
			set { __Offer_ID_Description = value; }
		}
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	#region Public Parent Tables
		...
		public KinguinProduct_To_Offers_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public KinguinProduct_To_Offers_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From KinguinProduct_To_Offers Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<KinguinProduct_To_Offers> GetKinguinProduct_To_Offers(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<KinguinProduct_To_Offers> _TmpReturn = new List<KinguinProduct_To_Offers>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From KinguinProduct_To_Offers Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new KinguinProduct_To_Offers((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<KinguinProduct_To_Offers> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<KinguinProduct_To_Offers> _TmpReturn = new List<KinguinProduct_To_Offers>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select KinguinProduct_To_Offers.ID From KinguinProduct_To_Offers Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select KinguinProduct_To_Offers.ID From KinguinProduct_To_Offers Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new KinguinProduct_To_Offers((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From KinguinProduct_To_Offers Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From KinguinProduct_To_Offers Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<KinguinProduct_To_Offers> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<KinguinProduct_To_Offers> _TmpReturn = new List<KinguinProduct_To_Offers>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select KinguinProduct_To_Offers.ID From KinguinProduct_To_Offers where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From KinguinProduct_To_Offers Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From KinguinProduct_To_Offers Where [" + FieldName + "] = @SearchParam";}
...
_Params.Add(new System.Data.SqlClient.SqlParameter("Offer_ID_Param", this.Offer_ID));
SQLInsertFields += "[Offer_ID], ";
SQLValues += "@Offer_ID_Param, ";
}
...
string _SQL = "Insert Into [KinguinProduct_To_Offers] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
if (this.Offer_ID != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("Offer_ID_Param", this.Offer_ID));
SQLInsertFields += "[Offer_ID] = @Offer_ID_Param, ";
}
if (this.DateAdded != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("DateAdded_Param", this.DateAdded));
...
string _SQL = "Update [KinguinProduct_To_Offers] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [KinguinProduct_To_Offers] Where " + SQLValues;
