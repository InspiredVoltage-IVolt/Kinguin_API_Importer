...
{
	public class KinguinProduct_To_Tags_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
	...
	private string _LocalConnectionString = "";
		...
		private int? __Tags_ID;
		private string __Tags_ID_Description = "";
	...
	public virtual string PrimaryKey { get { return "ID"; } }		public virtual int? ID
		...
		public virtual int? Tags_ID
		{
			get { return __Tags_ID; }
			set { __Tags_ID = value; }
		}

		public virtual string Tags_ID_Description
		{
			get { return __Tags_ID_Description; }
			set { __Tags_ID_Description = value; }
		}
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public Tags Get_Tags() { 
		...
		Tags _TmpReturn = null;
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		DataAccess.Open(GenericStaticClass.DatabaseConnectionString);
		List<System.Data.SqlClient.SqlParameter> _Params = new List<System.Data.SqlClient.SqlParameter>();
		_Params.Add(new System.Data.SqlClient.SqlParameter("Tags_IDParam", this.ReturnProperty("Tags_ID")));
		var SQL = "Select top 1 Tags.ID From Tags Where ID = @Tags_IDParam";
		var Result = DataAccess.RunCommand(SQL, _Params.ToList<System.Data.IDataParameter>(), true, System.Data.CommandType.Text);

		if (Result.Tables[0].Rows.Count == 1)
		{
			_TmpReturn = new Tags((int?)Result.Tables[0].Rows[0]["ID"]);
		}
	...
	return _TmpReturn;
		...
		public KinguinProduct_To_Tags_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public KinguinProduct_To_Tags_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From KinguinProduct_To_Tags Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<KinguinProduct_To_Tags> GetKinguinProduct_To_Tags(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		...
		var QR = DataAccess.RunCommand("Select [ID] From KinguinProduct_To_Tags Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new KinguinProduct_To_Tags((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<KinguinProduct_To_Tags> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<KinguinProduct_To_Tags> _TmpReturn = new List<KinguinProduct_To_Tags>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select KinguinProduct_To_Tags.ID From KinguinProduct_To_Tags Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select KinguinProduct_To_Tags.ID From KinguinProduct_To_Tags Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new KinguinProduct_To_Tags((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From KinguinProduct_To_Tags Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From KinguinProduct_To_Tags Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<KinguinProduct_To_Tags> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<KinguinProduct_To_Tags> _TmpReturn = new List<KinguinProduct_To_Tags>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select KinguinProduct_To_Tags.ID From KinguinProduct_To_Tags where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From KinguinProduct_To_Tags Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From KinguinProduct_To_Tags Where [" + FieldName + "] = @SearchParam";}
...
_Params.Add(new System.Data.SqlClient.SqlParameter("Tags_ID_Param", this.Tags_ID));
SQLInsertFields += "[Tags_ID], ";
SQLValues += "@Tags_ID_Param, ";
}
SQLInsertFields = SQLInsertFields.TrimEnd(", ".ToCharArray());
SQLValues = SQLValues.TrimEnd(", ".ToCharArray());
string _SQL = "Insert Into [KinguinProduct_To_Tags] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
if (this.Tags_ID != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("Tags_ID_Param", this.Tags_ID));
SQLInsertFields += "[Tags_ID] = @Tags_ID_Param, ";
}
SQLInsertFields = SQLInsertFields.TrimEnd(", ".ToCharArray());
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Update [KinguinProduct_To_Tags] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [KinguinProduct_To_Tags] Where " + SQLValues;
