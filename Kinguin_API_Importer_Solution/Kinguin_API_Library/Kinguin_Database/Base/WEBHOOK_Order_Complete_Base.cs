...
{
	public class WEBHOOK_Order_Complete_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	#region Public Parent Tables
		...
		public WEBHOOK_Order_Complete_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public WEBHOOK_Order_Complete_CoreClass(Guid? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From WEBHOOK_Order_Complete Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<WEBHOOK_Order_Complete> GetWEBHOOK_Order_Complete(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<WEBHOOK_Order_Complete> _TmpReturn = new List<WEBHOOK_Order_Complete>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From WEBHOOK_Order_Complete Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new WEBHOOK_Order_Complete((Guid?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<WEBHOOK_Order_Complete> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<WEBHOOK_Order_Complete> _TmpReturn = new List<WEBHOOK_Order_Complete>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select WEBHOOK_Order_Complete.ID From WEBHOOK_Order_Complete Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select WEBHOOK_Order_Complete.ID From WEBHOOK_Order_Complete Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new WEBHOOK_Order_Complete((Guid)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From WEBHOOK_Order_Complete Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From WEBHOOK_Order_Complete Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<WEBHOOK_Order_Complete> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<WEBHOOK_Order_Complete> _TmpReturn = new List<WEBHOOK_Order_Complete>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select WEBHOOK_Order_Complete.ID From WEBHOOK_Order_Complete where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From WEBHOOK_Order_Complete Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From WEBHOOK_Order_Complete Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [WEBHOOK_Order_Complete] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [WEBHOOK_Order_Complete] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [WEBHOOK_Order_Complete] Where " + SQLValues;
