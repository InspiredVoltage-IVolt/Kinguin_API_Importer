...
{
	public class KinguinProduct_To_Languages_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
	...
	private string _LocalConnectionString = "";
		...
		private int? __Languages_ID;
		private string __Languages_ID_Description = "";
	...
	public virtual string PrimaryKey { get { return "ID"; } }		public virtual int? ID
		...
		public virtual int? Languages_ID
		{
			get { return __Languages_ID; }
			set { __Languages_ID = value; }
		}

		public virtual string Languages_ID_Description
		{
			get { return __Languages_ID_Description; }
			set { __Languages_ID_Description = value; }
		}
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	public Languages Get_Languages() { 
		...
		Languages _TmpReturn = null;
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		DataAccess.Open(GenericStaticClass.DatabaseConnectionString);
		List<System.Data.SqlClient.SqlParameter> _Params = new List<System.Data.SqlClient.SqlParameter>();
		_Params.Add(new System.Data.SqlClient.SqlParameter("Languages_IDParam", this.ReturnProperty("Languages_ID")));
		var SQL = "Select top 1 Languages.ID From Languages Where ID = @Languages_IDParam";
		var Result = DataAccess.RunCommand(SQL, _Params.ToList<System.Data.IDataParameter>(), true, System.Data.CommandType.Text);

		if (Result.Tables[0].Rows.Count == 1)
		{
			_TmpReturn = new Languages((int?)Result.Tables[0].Rows[0]["ID"]);
		}
	...
	return _TmpReturn;
		...
		public KinguinProduct_To_Languages_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public KinguinProduct_To_Languages_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From KinguinProduct_To_Languages Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<KinguinProduct_To_Languages> GetKinguinProduct_To_Languages(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		...
		var QR = DataAccess.RunCommand("Select [ID] From KinguinProduct_To_Languages Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new KinguinProduct_To_Languages((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<KinguinProduct_To_Languages> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<KinguinProduct_To_Languages> _TmpReturn = new List<KinguinProduct_To_Languages>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select KinguinProduct_To_Languages.ID From KinguinProduct_To_Languages Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select KinguinProduct_To_Languages.ID From KinguinProduct_To_Languages Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new KinguinProduct_To_Languages((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From KinguinProduct_To_Languages Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From KinguinProduct_To_Languages Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<KinguinProduct_To_Languages> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<KinguinProduct_To_Languages> _TmpReturn = new List<KinguinProduct_To_Languages>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select KinguinProduct_To_Languages.ID From KinguinProduct_To_Languages where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From KinguinProduct_To_Languages Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From KinguinProduct_To_Languages Where [" + FieldName + "] = @SearchParam";}
...
if (this.Languages_ID != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("Languages_ID_Param", this.Languages_ID));
SQLInsertFields += "[Languages_ID], ";
SQLValues += "@Languages_ID_Param, ";
}
SQLInsertFields = SQLInsertFields.TrimEnd(", ".ToCharArray());
SQLValues = SQLValues.TrimEnd(", ".ToCharArray());
string _SQL = "Insert Into [KinguinProduct_To_Languages] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
if (this.Languages_ID != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("Languages_ID_Param", this.Languages_ID));
SQLInsertFields += "[Languages_ID] = @Languages_ID_Param, ";
}
SQLInsertFields = SQLInsertFields.TrimEnd(", ".ToCharArray());
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Update [KinguinProduct_To_Languages] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [KinguinProduct_To_Languages] Where " + SQLValues;
