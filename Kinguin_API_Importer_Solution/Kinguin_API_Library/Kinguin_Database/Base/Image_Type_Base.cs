...
{
	public class Image_Type_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	#region Public Parent Tables
		...
		public Image_Type_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Image_Type_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Image_Type Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Image_Type> GetImage_Type(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Image_Type> _TmpReturn = new List<Image_Type>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Image_Type Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Image_Type((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Image_Type> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Image_Type> _TmpReturn = new List<Image_Type>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Image_Type.ID From Image_Type Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Image_Type.ID From Image_Type Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Image_Type((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Image_Type Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Image_Type Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Image_Type> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Image_Type> _TmpReturn = new List<Image_Type>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Image_Type.ID From Image_Type where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Image_Type Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Image_Type Where [" + FieldName + "] = @SearchParam";}
...
string _SQL = "Insert Into [Image_Type] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
string _SQL = "Update [Image_Type] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Image_Type] Where " + SQLValues;
