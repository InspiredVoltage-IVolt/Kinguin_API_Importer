...
{
	public class Screenshots_CoreClass : ACT.Plugins.ACT_Core, ACT.Core.Interfaces.DataAccess.I_DataObject 
	{
	...
	private string _LocalConnectionString = "";
		...
		private string __URL_Original;
		private string __URL_Original_Description = "";
	...
	public virtual string PrimaryKey { get { return "ID"; } }		public virtual int? ID
		...
		public virtual string URL_Original
		{
			get { return __URL_Original; }
			set { __URL_Original = value; }
		}

		public virtual string URL_Original_Description
		{
			get { return __URL_Original_Description; }
			set { __URL_Original_Description = value; }
		}
...
public static ACT.Core.Interfaces.DataAccess.I_DataAccess GetDataAccess()
{
	...
	#region Public Parent Tables
		...
		public Screenshots_CoreClass(string localConnectionString = ""){ LocalConnectionString = localConnectionString; }


		public Screenshots_CoreClass(int? ID, string localConnectionString = "") 
		{
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		...
		var SQL = "Select * From Screenshots Where [ID] = @IDParam";
	...
	#region Public Static Search Methods
		...
		public static List<Screenshots> GetScreenshots(I_DbWhereStatement Where, string sql = "", string LocalConnectionString = "")
		{
		List<Screenshots> _TmpReturn = new List<Screenshots>();
		using(var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
		if (LocalConnectionString == "") {
		...
		var QR = DataAccess.RunCommand("Select [ID] From Screenshots Where " + _Where, _Params, true, System.Data.CommandType.Text);
		if (QR.Tables[0].Rows.Count > 0)
		{
		    foreach (System.Data.DataRow r in QR.Tables[0].Rows)
		    {
		        _TmpReturn.Add(new Screenshots((int?)r["ID"]));
		    }
		}
return _TmpReturn;
		}}

public static List<Screenshots> Search(string FieldName, string SearchText, bool UseContains = false, string LocalConnectionString = "")
{
List<Screenshots> _TmpReturn = new List<Screenshots>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select Screenshots.ID From Screenshots Where [" + FieldName + "] like '%' + @SearchParam + '%'";
} 
else { 
SQL = SQL + "Select Screenshots.ID From Screenshots Where [" + FieldName + "] = @SearchParam";
}
...
{
_TmpReturn.Add(new Screenshots((int)Result.Tables[0].Rows[x]["ID"]));
}
}
}}
return _TmpReturn;
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Screenshots Where [" + FieldName + "] like '%' + @SearchParam + '%') as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
} 
else { 
SQL = SQL + "Select * From ( Select *, ROW_NUMBER() Over(order by " + OrderByStatement + ") as RowNum From Screenshots Where [" + FieldName + "] = @SearchParam) as T Where T.RowNum > " + (Start-1).ToString() + " AND T.RowNum < " + (Start + Increment).ToString();
}
...
public static List<Screenshots> Search(Dictionary<string,string> FieldValues, string LocalConnectionString = "")
{
List<Screenshots> _TmpReturn = new List<Screenshots>();
using (var DataAccess = ACT.Core.CurrentCore<ACT.Core.Interfaces.DataAccess.I_DataAccess>.GetCurrent()){
...
var SQL = "";
SQL = SQL + "Select Screenshots.ID From Screenshots where ";
...
var SQL = "";
if (UseContains == true) { 
SQL = SQL + "Select * From Screenshots Where [" + FieldName + "] like '%' + @SearchParam + '%'";} 
else { 
SQL = SQL + "Select * From Screenshots Where [" + FieldName + "] = @SearchParam";}
...
if (this.URL_Original != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("URL_Original_Param", this.URL_Original));
SQLInsertFields += "[URL_Original], ";
SQLValues += "@URL_Original_Param, ";
}
...
string _SQL = "Insert Into [Screenshots] ( " + SQLInsertFields + ") Values (" + SQLValues + ")";
...
if (this.URL_Original != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("URL_Original_Param", this.URL_Original));
SQLInsertFields += "[URL_Original] = @URL_Original_Param, ";
}
if (this.DateAdded != null){ 
_Params.Add(new System.Data.SqlClient.SqlParameter("DateAdded_Param", this.DateAdded));
...
string _SQL = "Update [Screenshots] set " + SQLInsertFields + " Where " + SQLValues;
var _TmpReturn = DataAccess.RunCommand(_SQL, _Params.ToList<System.Data.IDataParameter>(), false, System.Data.CommandType.Text);
...
SQLValues = SQLValues.TrimEnd("AND ".ToCharArray());
string _SQL = "Delete From [Screenshots] Where " + SQLValues;
